...
const visObject={options:{font_size:{type:"string",label:"Font Size",values:[{Large:"large"},{Small:"small"}],display:"radio",default:"large"}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>";t.innerHTML=`
    <style>
    .text-large {
      font-size: 72px;
    }
    ...
    </style>`;t=t.appendChild(document.createElement("div"));t.className="text-large",this._textElement=t.appendChild(document.createElement("div"))},updateAsync:function(t,l,e,a,n,s){const i=a.fields.dimension_like,o=a.fields.measure_like[0],r=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!e.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){d<+a[o.name].value&&(d=+a[o.name].value,l.innerHTML=`<h1>Max : ${d}</h1>`),r.push({index:r.length,name:n.join("-"),value:+a[o.name].value})})}),console.log("final chart data",r);var m={top:30,right:30,bottom:70,left:60},c=l.clientWidth,p=l.clientHeight;const h=d3.select("#vis").append("svg").attr("width",c+m.left+m.right).attr("height",p+m.top+m.bottom).append("g").attr("transform","translate("+m.left+","+m.top+")"),f=d3.scaleBand().range([0,c]).domain(r.map(t=>t.index)).padding(1);h.append("g").attr("transform","translate(0,"+p+")").call(d3.axisBottom(f).tickFormat(t=>r[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const u=d3.scaleLinear().domain([0,d3.max(r,t=>t.value)]).range([p,0]);h.append("g").call(d3.axisLeft(u)),h.append("path").datum(r).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>f(t.index)).y0(u(0)).y1(t=>u(t.value))),"small"==e.font_size?this._textElement.className="text-small":this._textElement.className="text-large";a=t[0][a.fields.dimensions[0].name];this._textElement.innerHTML=LookerCharts.Utils.htmlForCell(a),s()}};looker.plugins.visualizations.add(visObject);
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,o,e,a,n,l){const i=a.fields.dimension_like,r=a.fields.measure_like[0],s=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!e.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){d<+a[r.name].value&&(d=+a[r.name].value,o.innerHTML=`<h1>Max : ${d}</h1>`),s.push({index:s.length,name:n.join("-"),value:+a[r.name].value})})}),console.log("final chart data",s);var c={top:30,right:30,bottom:70,left:60},a=o.clientWidth,t=o.clientHeight;const p=d3.select("#vis").append("svg").attr("width",a+c.left+c.right).attr("height",t+c.top+c.bottom).append("g").attr("transform","translate("+c.left+","+c.top+")"),u=d3.scaleBand().range([0,a]).domain(s.map(t=>t.index)).padding(1);p.append("g").attr("transform","translate(0,"+t+")").call(d3.axisBottom(u).tickFormat(t=>s[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const f=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([t,0]);p.append("g").call(d3.axisLeft(f)),p.append("path").datum(s).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>u(t.index)).y0(f(0)).y1(t=>f(t.value))),l()}};looker.plugins.visualizations.add(visObject);
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,o,e,a,n,l){const i=a.fields.dimension_like,r=a.fields.measure_like[0],s=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!e.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){d<+a[r.name].value&&(d=+a[r.name].value,o.innerHTML=`<h1>Max : ${d}</h1>`),s.push({index:s.length,name:n.join("-"),value:+a[r.name].value})})}),console.log("final chart data",s);var c={top:30,right:30,bottom:70,left:60},a=o.clientWidth,t=o.clientHeight;const p=d3.select("#vis").append("svg").attr("width",a+c.left+c.right).attr("height",t+c.top+c.bottom).append("g").attr("transform","translate("+c.left+","+c.top+")"),u=d3.scaleBand().range([0,a]).domain(s.map(t=>t.index)).padding(1);p.append("g").attr("transform","translate(0,"+t+")").call(d3.axisBottom(u).tickFormat(t=>s[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const f=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([t,0]);p.append("g").call(d3.axisLeft(f)),p.append("path").datum(s).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>u(t.index)).y0(f(0)).y1(t=>f(t.value))),l()}};looker.plugins.visualizations.add(visObject);
